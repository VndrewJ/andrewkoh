export const longDesc = {
    MIDI_CTRL: 
`This project is a small scale version of one I've wanted to build for years as a means to learn **RTOS** development and **matrix scanning** for devices with a high amount of inputs to build a fully fleshed out version of it later on.
...
The biggest challenge I found in this project was getting the BLE stack to work reliably, especially with multiple concurrent tasks running on the device. I had to do a lot of research in finding out the configuration required for the ESP32 to work with various OS BLE stacks (Windows, MACOS,IOS) which then influenced how I implemented task scheduling in RTOS. 
...
Ultimately however, this project highlighted the gap between machine learning and human-like understanding in building artificial intelligence. 
`,
    TELE_OXI:
...
Looking back on the project, I think our biggest area of improvement would be our use of blocking code. Many parts of our code were blocking or used a lot of resource in float calculations, which could have been the cause of a lot of our sensor reading issues and jittery behaviour.
...
`,
    ...
    ROBOT_HAND:
` The main project of the MECHENG 736: Biomechatronic Systems course was to design and build a robotic gripper, actuated by a singular dynamixel servo, to pick up a variety of objects ranging from small and thin, to large and heavy. 
...
For our group, we decided to go with a 5 finger, double jointed, **anthropomorphic** design with a whipple tree mechanism to evenly distribute the force of the motor across all fingers. CAD was done in **Onshape** and parts were 3D printed in PLA with rubber molds used for hand/finger pads to increase friction.
...
Other designs we considered was the use of TPU plastic for the fingers, which could have provided a more flexible and compliant design, as well as a more traditional parallel jaw gripper design, which would have been simpler to build and control, but less novel and anthropomorphic.
`,
    WELD_GAP:
` The 2nd project in MECHENG 709: Industrial Automation was to put all the theory we had learned about **computer vision** to use in building a system that could detect the gap in a weld and adjust the position of the welding torch accordingly to ensure a proper weld.
`,
}
